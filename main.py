...
def parse_args():
    ...
    parser.add_argument('--phase', type=str, choices=['train', 'test', 'video', 'camera', 'img_dir'],
                        help='[train | test | video | camera | img_dir ]')
    parser.add_argument('--light', type=int, default=-1, help='[U-GAT-IT full version / U-GAT-IT light version]')
    parser.add_argument('--dataset', type=str, default='YOUR_DATASET_NAME', help='dataset_name')

    parser.add_argument('--iteration', type=int, default=1000000, help='The number of training iterations')
    parser.add_argument('--batch_size', type=int, default=1, help='The size of batch size')
    parser.add_argument('--num_workers', type=int, default=2, help='每个 DataLoader 的进程数')
    ...
    parser.add_argument('--sn', action='store_true', help='默认D网络不使用sn，可以使用')
...
def main():
    ...
    if args.phase in ('video', 'camera', 'img_dir'):
        ...
        generator = ResnetGenerator(input_nc=3, output_nc=3, ngf=args.ch, n_blocks=args.n_res,
                                    img_size=args.img_size, light=args.light,
                                    attention_gan=args.attention_gan,
                                    use_se=args.use_se, use_deconv=args.use_deconv).to(args.device)
